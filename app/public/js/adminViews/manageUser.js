...
function handleEditUser(event) {
    ...
    cells.forEach((cell, index) => {
        if (index === 2) { // Check if it's the userRole column so we can replace the text with a select element
        ...
        } else if (index !== 0 && index !== 3) { // Skip the ID and registration date columns from editing
            const currentValue = cell.textContent.trim();
...
function resetTable() {
    ...
    editButtons.forEach(button => {
        ...
        deleteButton.style.cssText = '--bs-btn-padding-y: .75rem; --bs-btn-padding-x: .75rem; --bs-btn-font-size: 1rem; border-radius: 10px;';
    ...
    const selectCells = document.querySelectorAll('td:not(.button-column) select');
